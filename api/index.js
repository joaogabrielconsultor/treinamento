...
app.post('/api/proposta/gerar', auth, async (req, res) => {
  try {
    ...
    const maskCPF = (c) => { const d = (c||'').replace(/\D/g,''); return d.length===11 ? `${d.slice(0,3)}.***.****-${d.slice(9)}` : c; };

    const tplPath = path.join(__dirname, 'templates', 'proposta_template.pdf');
    if (!fs.existsSync(tplPath)) return res.status(404).json({ error: 'Template PDF não encontrado no servidor' });

    const pdfDoc = await PDFDocument.load(fs.readFileSync(tplPath));
    const page = pdfDoc.getPages()[0];
    const { width, height } = page.getSize(); // 810 x 787.5
    ...
    function drawRight(text, xRight, y, size, font, color) {
      const w = font.widthOfTextAtSize(text, size);
      page.drawText(text, { x: xRight - w, y, size, font, color });
    }

    // ── Corretor photo — cobre a foto do Fernando ──
    const avatarDir = path.join(UPLOADS_DIR, 'avatars');
    for (const ext of ['.jpg', '.jpeg', '.png', '.webp']) {
      const p = path.join(avatarDir, `${req.user.id}${ext}`);
    ...
    page.drawText(fmtNum(valorLiquido), {
      x: 190, y: height - 293, size: 40, font: boldFont, color: GREEN,
    });

    // ── Box 2: Parcela utilizada ──
    page.drawText(fmtNum(parcela), {
      x: 552, y: height - 338, size: 46, font: boldFont, color: GREEN,
    });
    ...
    page.drawText(fmtNum(valorDivida), {
      x: 190, y: height - 515, size: 40, font: boldFont, color: GREEN,
    });

    // ── Box 4: Banco responsável (coluna direita) ──
    page.drawText(bancoResponsavel, {
      x: 479, y: height - 538, size: 46, font: boldFont, color: GREEN,
    });

    const pdfBytes = await pdfDoc.save();
    const filename = `Proposta_${(nomeCliente||'Cliente').replace(/\s+/g,'_')}.pdf`;
...
// Relatório de comissões por corretor x mês, com detalhe de cada proposta (paga + pendente)
app.get('/api/admin/commission-report', auth, adminOnly, async (req, res) => {
  const { rows } = await pool.query(`
    SELECT p.id, p.proposal_number, p.value, p.status, p.status_comissao, p.updated_at,
           p.client_name, p.bank, ft.name as table_name,
           u.id as user_id, u.full_name as user_name, u.email as user_email,
           COALESCE(p.comissao_corretor_override,
             ROUND(p.value * COALESCE(
               (SELECT cr.comissao_corretor FROM commission_ranges cr
  ...
  for (const r of rows) {
    const corretor = parseFloat(r.comissao_corretor) || 0;
    const empresa = parseFloat(r.comissao_empresa) || 0;
    const isPendente = r.status_comissao === 'Ag. Comissão';
    const month = r.updated_at.toISOString().slice(0, 7);

    if (!brokersMap.has(r.user_id)) {
      brokersMap.set(r.user_id, {
        user_id: r.user_id, user_name: r.user_name, user_email: r.user_email,
        by_month: {},
        total_corretor_pendente: 0, total_empresa_pendente: 0,
        total_corretor_pago: 0, total_empresa_pago: 0,
        proposals: [],
      });
    }
    const b = brokersMap.get(r.user_id);
    b.proposals.push({
      id: r.id, proposal_number: r.proposal_number, client_name: r.client_name,
      bank: r.bank, table_name: r.table_name, value: parseFloat(r.value),
      comissao_corretor: corretor, comissao_empresa: empresa,
      status: r.status, status_comissao: r.status_comissao, updated_at: r.updated_at,
    });
    if (isPendente) {
      monthsSet.add(month);
      if (!b.by_month[month]) b.by_month[month] = { corretor: 0, empresa: 0 };
      b.by_month[month].corretor += corretor;
      b.by_month[month].empresa += empresa;
      b.total_corretor_pendente += corretor;
      b.total_empresa_pendente += empresa;
    } else {
      b.total_corretor_pago += corretor;
      b.total_empresa_pago += empresa;
    }
  }

  const months = [...monthsSet].sort();
  const brokers = [...brokersMap.values()].sort((a, b) =>
    (b.total_corretor_pendente + b.total_empresa_pendente) - (a.total_corretor_pendente + a.total_empresa_pendente)
  );

  const totals = {
    by_month: {}, total_corretor_pendente: 0, total_empresa_pendente: 0,
    total_corretor_pago: 0, total_empresa_pago: 0,
  };
  for (const m of months) totals.by_month[m] = { corretor: 0, empresa: 0 };
  for (const b of brokers) {
    totals.total_corretor_pendente += b.total_corretor_pendente;
    totals.total_empresa_pendente += b.total_empresa_pendente;
    totals.total_corretor_pago += b.total_corretor_pago;
    totals.total_empresa_pago += b.total_empresa_pago;
