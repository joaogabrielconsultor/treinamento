...
interface FilterUser { id: string; full_name: string | null; email: string; loja_id?: string | null; }
...
// ── Número animado ──
function AnimCount({ to, fmt = 'currency', delay = 0 }: { to: number; fmt?: 'currency' | 'percent' | 'int'; delay?: number }) {
  const v = useCountUp(to, 900, delay);
  if (fmt === 'currency') return <>{fmtR(v)}</>;
...
function Card({ children, className = '', delay = 0 }: { children: React.ReactNode; className?: string; delay?: number }) {
  return (
    <div
      ...
      style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', boxShadow: 'var(--shadow-card)', animationDelay: `${delay}ms` }}
    >
      {children}
    </div>
  );
...
// ── KPI stat card ──
function KPI({ label, value, sub, icon: Icon, color, delay = 0 }: {
...
// ── Barra horizontal com animação própria ──
function AggBar({ item, maxVal, color, metric, index = 0 }: {
  ...
  const display =
    ...
    metric === 'ticket'     ? fmtR(item.ticket) :
    metric === 'commission' ? fmtR(item.commission) :
    fmtR(item.value);
...
export function Production({ isAdmin }: { isAdmin: boolean }) {
  ...
  async function loadData() {
    ...
    else if (isAdmin && filterLoja) pp.set('loja_id', filterLoja);
  ...
  useEffect(() => {
    if (isAdmin) {
      ...
      API('/api/admin/lojas/all').then(r => r.json()).then(d => { if (Array.isArray(d)) setLojas(d); });
    }
  }, [isAdmin]);

  useEffect(() => { loadData(); }, [period, dateFrom, dateTo, filterCorretor, filterLoja]);
  ...
  const corretoresFiltrados = useMemo(
    () => filterLoja ? corretores.filter(c => c.loja_id === filterLoja) : corretores,
    [corretores, filterLoja]
  );
  ...
  const dimData = useMemo(() => {
    const base =
      dim === 'bank'         ? aggBank :
      dim === 'convenio'     ? aggConv :
      dim === 'product'      ? aggProd :
      dim === 'table'        ? aggTab  :
      dim === 'tipo'         ? aggTipo : aggUB;
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="text-center">
        ...
        <p className="text-sm" style={{ color: 'var(--text-3)' }}>Carregando análises...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto" style={{ color: 'var(--text-1)' }}>
      ...
      <div className="flex items-center justify-between mb-6 animate-fade-up">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-3)' }}>
            {isAdmin ? 'Visão completa da equipe' : 'Sua performance detalhada'}
          </p>
        </div>
      ...
      <div key={tab} className="animate-fade-up" style={{ animationDuration: '0.35s' }}>
        ...
        {tab === 'overview' && (
          <div className="space-y-5">
            <div className={`grid grid-cols-2 lg:grid-cols-5 gap-4`}>
              ...
              {isAdmin
                ? <KPI label="Comissão Gerada" value={<AnimCount to={comm.corrTotal} fmt="currency" delay={320} />} sub="total corretores" icon={Percent} color="#60a5fa" delay={240} />
                : <KPI label="Minha Comissão"  value={<AnimCount to={stats?.my_commission_total || 0} fmt="currency" delay={320} />} sub={`#${stats?.my_position || '—'} no ranking`} icon={Star} color="#60a5fa" delay={240} />
              }
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              <Card delay={160}>
                ...
                <div className="flex gap-1.5 mb-4">
                  {last7.map(d => (
                    <div key={d.ds} className="flex-1 text-center">
                      <p className="text-[9px] truncate" style={{ color: 'var(--text-3)' }}>{d.label}</p>
                    </div>
                  ))}
                </div>
          ...
          <div className="space-y-5">
            ...
            <Card delay={80}>
              {dimData.length === 0
                ...
                : (
                  <>
                    <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider pb-3 mb-1"
                      style={{ color: 'var(--text-3)', borderBottom: '1px solid var(--card-border)' }}>
                      <span>Dimensão</span>
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
              {[
                ...
                <div key={label} className="rounded-xl p-4 text-center animate-fade-up"
                  style={{ background: `${color}08`, border: `1px solid ${color}20`, animationDelay: `${120 + i * 50}ms` }}>
                ...
                : aggBrok.map((b, i) => {
                  ...
                  return (
                    <div key={b.label} className="py-3" style={i > 0 ? { borderTop: '1px solid var(--card-border)' } : {}}>
                      ...
                      <AggBar item={b} maxVal={aggBrok[0].value} color={mc} metric="value" index={i} />
                    </div>
                  );
                })
              }
          ...
          <div className="space-y-5">
            ...
            <Card delay={220}>
              ...
              {aggBank.slice(0, 8).map((b, i) => (
                <AggBar key={b.label} item={b} maxVal={aggBank[0]?.commission || 1} color="#60a5fa" metric="commission" index={i} />
              ))}
            </Card>
          </div>
        )}
                ...
                {streak && (
                  <Card delay={60}>
                    <div className="flex items-center gap-2 mb-4">
                      <Zap className="w-4 h-4 animate-pulse-glow" style={{ color: '#fbbf24' }} />
                      <h3 className="text-sm font-semibold" style={{ color: 'var(--text-1)' }}>Streak Diária</h3>
                    </div>
                    <p className="text-5xl font-black num mb-2" style={{ color: '#fbbf24' }}>
                      🔥 <AnimCount to={streak.current_streak} fmt="int" delay={100} />
                    </p>
                    <p className="text-sm" style={{ color: 'var(--text-3)' }}>dias consecutivos com proposta paga</p>
                    <div className="grid grid-cols-2 gap-3 mt-5">
                      {[
                        { label: 'Streak atual', value: streak.current_streak, color: '#fbbf24' },
                        { label: 'Recorde',      value: streak.best_streak,    color: '#94a3b8' },
                      ].map(({ label, value, color }) => (
                        <div key={label} className="rounded-xl p-3 text-center"
                          style={{ background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.2)' }}>
                          <p className="text-[10px]" style={{ color: 'var(--text-3)' }}>{label}</p>
                          <p className="text-xl font-black num" style={{ color }}>{value}d</p>
                        </div>
                      ))}
                    </div>
                  </Card>
                      ...
                      {[
                        ...
                        return (
                          <div key={label}>
                            ...
                            <div className="h-3 rounded-full" style={{ background: 'rgba(255,255,255,0.05)' }}>
                              <StatusBar pct={pct} cls={cls} delay={d} />
                            </div>
                          </div>
                        );
                      })}
            ...
            {badges.length > 0 && (
              <Card delay={80}>
                <div className="flex items-center gap-3 mb-5">
                  <Award className="w-4 h-4 animate-float" style={{ color: '#fbbf24' }} />
                    ...
                    <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2 mb-6">
                      {badges.filter(b => b.earned).map((b, i) => (
                        <div key={b.id} title={`${b.name}: ${b.description}`}
                          ...
                          style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', boxShadow: '0 0 12px rgba(245,158,11,0.08)', animationDelay: `${100 + i * 30}ms` }}>
                    ...
                    <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2">
                      {badges.filter(b => !b.earned).map((b, i) => (
                        <div key={b.id} title={`${b.name}: ${b.description}`}
                          ...
                          style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.10)', filter: 'grayscale(0.75)', animationDelay: `${150 + i * 25}ms` }}>
                          <span className="text-xl badge-icon">{b.icon}</span>
            ...
            {isAdmin && ranking.length > 0 && (
              <Card delay={140}>
                ...
                {ranking.slice(0, 10).map((r, i) => {
                  const pct = ranking[0].total_points > 0 ? (r.total_points / ranking[0].total_points) * 100 : 0;
                  return (
                    <div key={r.user_id} className="py-2.5 animate-fade-up"
                      style={i > 0 ? { borderTop: '1px solid var(--card-border)', animationDelay: `${120 + i * 40}ms` } : { animationDelay: `${120 + i * 40}ms` }}>
                      <div className="flex items-center gap-3 mb-1">
                        <span className="w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-black flex-shrink-0"
                          style={{ background: i < 3 ? 'rgba(251,191,36,0.15)' : 'rgba(255,255,255,0.05)', color: i < 3 ? '#fbbf24' : '#475569' }}>
                          {r.position}
