...
export function AdminContaCorrente({ isMaster = false }: { isMaster?: boolean }) {
  ...
  async function saveDespesa() {
    ...
    await API('/api/admin/despesas', {
      ...
      body: JSON.stringify({
        ...
        valor: despesaForm.valor.includes(',') ? parseFloat(despesaForm.valor.replace(/\./g, '').replace(',', '.')) : parseFloat(despesaForm.valor),
  ...
  async function saveEdit(id: string) {
    setSavingEdit(true);
    const parseVal = (v: string) => v.trim() === '' ? null : (v.includes(',') ? parseFloat(v.replace(/\./g, '').replace(',', '.')) : parseFloat(v)) || 0;
